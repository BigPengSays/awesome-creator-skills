...
function coverageLine(skills: Skill[], taxonomy: Taxonomy): string {
  ...
  return `**${skills.length}** 个 skill · ${platformPart}${typePart ? `\n\n${typePart}` : ""}`;
}

function introBlock(skills: Skill[], taxonomy: Taxonomy): string {
  return `> **Agent Skills** 精选目录，给内容创作者 — 写文章、做封面、剪视频、做 PPT，一条命令装进 Cursor、Claude Code。

按平台和创作类型整理，少踩坑、少搜 repo。

${coverageLine(skills, taxonomy)}

> **持续更新中** · [**欢迎投稿**](CONTRIBUTING.md)（有好用的 skill，提 Issue 或 PR）`;
}
